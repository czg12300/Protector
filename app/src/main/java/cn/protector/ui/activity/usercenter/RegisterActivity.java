...
public class RegisterActivity extends CommonTitleActivity
    ...
    private static final int MSG_UI_SEND_MESSAGE_CODE = 2;
    ...
    public void handleBackgroundMessage(Message msg) {
        ...
        switch (msg.what) {
            case MSG_BACK_REGISTER:
                HttpRequest<CommonResponse> request = new HttpRequest<>(AppConfig.REGISTER,
                ...
                break;
            // case MSG_BACK_LOGIN:
            // HttpRequest<LoginResponse> rLogin = new
            // HttpRequest<>(AppConfig.LOGIN,
            // LoginResponse.class);
            // if (mEvMobile != null) {
     ...
     * 发送注册成功的广播
     */
    private void sendRegisterSuccessBroadcast() {
        if (mEvMobile != null && mEvPw != null) {
            Intent it = new Intent(BroadcastActions.ACTION_REGISTER_SUCCESS);
            it.putExtra("mobile", mEvMobile.getText().toString());
            it.putExtra("password", mEvPw.getText().toString());
            sendBroadcast(it);
        }
    }
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        ...
        if (!TextUtils.isEmpty(mEvMobile.getText()) && !TextUtils.isEmpty(mEvCode.getText())
                && !TextUtils.isEmpty(mEvPw.getText())) {
            mBtnOk.setEnabled(true);
        } else {
            mBtnOk.setEnabled(false);
        }
