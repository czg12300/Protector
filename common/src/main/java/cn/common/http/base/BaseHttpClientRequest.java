...
public abstract class BaseHttpClientRequest<T> {
  ...
  public T requestWithoutCheck(boolean isRetry) throws AppException {
    ...
    do {
      try {
        ...
        if (statusCode != HttpStatus.SC_OK && statusCode != HttpStatus.SC_CREATED && statusCode != HttpStatus.SC_NO_CONTENT && statusCode != HttpStatus.SC_PARTIAL_CONTENT) {
          throw AppException.http(statusCode);
        }
        String result = EntityUtils.toString(new BufferedHttpEntity(httpResponse.getEntity()), HTTP.UTF_8);
        if (isDebug) {
  ...
  private HttpUriRequest createHttpUriRequest() throws AppException, UnsupportedEncodingException {
    ...
    if (isGet) {
      ...
      ((HttpPost) uriReq).setEntity(isFile ? getFilePostRequestEntity() : getPostRequestEntity());
    }
    // 设置头
  ...
  protected HttpEntity getFilePostRequestEntity() {
...
//    for (String key : keys) {
//      try {
//        mpEntity.addPart(key, new StringBody(mParams.get(key)));
//      } catch (UnsupportedEncodingException e) {
//        e.printStackTrace();
//      }
//    }
    ...
    if (!TextUtils.isEmpty(filePath)) {
      mpEntity.addPart("file", new FileBody(new File(filePath), "image/jpeg"));
    }
    return mpEntity;
  }
  ...
  private String getGetRequestParams() throws UnsupportedEncodingException {
    if (mParams != null && mParams.size() >= 0) {
      ...
      final Set<String> keys = mParams.keySet();
      for (String key : keys) {
        builder.append(key).append("=").append(URLEncoder.encode(mParams.get(key), "UTF-8")).append("&");
      }
